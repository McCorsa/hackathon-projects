<script lang="ts">
    ...
    const unlockDate = new Date(2023, 6, 14, 19, 0, 0);
    let unlocked = nowDate >= unlockDate;
...
{:else}
    <main class="h-screen flex flex-col items-center justify-center gap-2 text-center p-4">
        <h1 class="text-2xl md:text-3xl">You have reached the Code Network Winter Hackathon registration site! 🥳</h1>
        <h1 class="text-2xl md:text-3xl">Unfortunately, registration for the Hackathon is not open yet.</h1>
        <h1 class="text-2xl md:text-3xl">Please check back in at <span class="text-primary font-semibold">7pm on Friday 14/07/23 (AEST)</span></h1>
    </main>
{/if}